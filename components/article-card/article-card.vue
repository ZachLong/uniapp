<template>
	<view :class="{ 'article-card-full': isFull === true || isFull === 'true', 'article-card-shadow': isShadow === true || isShadow === 'true' }"
	 class="article-card " @click="onClick">
		<navigator v-if="mode === 'basic'" class="article_link clearfix">
			<view class="item_detail">
				<h3 class="line3 dotdot">{{ title }}</h3>
				<view class="item_info">
					<text class="space">{{ author }}</text>
					<text class="space">评论 {{ number }}</text>
					<text>{{ time }}</text>
				</view>
			</view>
		</navigator>
		<navigator v-if="mode === 'single'" class="article_link clearfix middle_mode">
		...
		<navigator v-if="mode === 'large'" class="article_link clearfix">
			<view class="item_detail">
				<h3 class="line3 dotdot">{{ title }}</h3>
		...
		<navigator v-if="mode === 'triple'" class="article_link clearfix">
			<view class="item_detail">
				<h3 class="line3 dotdot">{{ title }}</h3>
				<view class="list_image">
					<ul v-if="cover" class="clearfix">
						<li v-for="(item, index) in imageList" :key="index" class="list_img_holder"><img :src="item.path" /></li>
					</ul>
				</view>
				<view class="item_info">
					<text class="space">{{ author }}</text>
...
<style scoped>
	...
	.article-card .list_image ul li:first-child {
		float: left;
		padding-right: 2px;
	}


	.article-card .list_image ul li:last-child {
		float: right;
		padding-left: 2px;
	}
	...
	.article-card .list_img_holder img {
		border: none;
		display: block;
		width: 100%;
		height: 100%;
	...
	.article-card .list_img_holder_large img {
		border: none;
		display: block;
		width: 100%;
		pointer-events: none;
