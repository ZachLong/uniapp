<template>
	<view class="register">
		...
		<view class="content">			
			...
			<!-- 主体 -->
			<view class="main">
				<wInput v-model="phoneData" type="text" maxlength="11" placeholder="请输入手机号作为账号"></wInput>
				<wInput v-model="passData" type="password" maxlength="20" placeholder="请设置密码" isShowPass></wInput>
			</view>
	...
	export default {
		...
		methods: {
			...
			startReg() {
				...
				uni.request({
					url: 'http://www.jinshenglan.com/api/v1/user/register', //仅为示例，并非真实接口地址。
					method: 'POST',
					data: {
					...
					success: (res) => {
						if(res.statusCode == 200){
							...
							this.$store.commit('login', res.data.registered)
							uni.reLaunch({
							    url: '../profile/profile',
								fail: (res) => {
									uni.showToast({
